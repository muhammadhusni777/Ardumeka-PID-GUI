...
def timerEvent():
    ...
    #########sensor filter###############################
    sensor_filtered = ((1-filter_weight) * sensor) + (filter_weight * sensor_filtered)
    ...
    print(str(" p ") +str(round(p_control,1)) + str(" | i : ")+str(i_control) + str(" | d : ")+str(d_control)
          + str(" dt : ")+str(dt) + str(" pid : ")+str(int(motor_command)) + str(" motor: ") + str(motor_speed) )
    
    
    
    dt_prev = time.time()
    client.publish("speed", str(motor_speed))
    error_prev = error
